...
public class ApplicationEnvironment {
  ...
  public <T> List<T> getExtensions(Class<T> type) {
    ...
    for (Class<? extends T> nextSubType : subTypes) {
      if (!Modifier.isAbstract(nextSubType.getModifiers())) {
        try {

          T instance = (T) nextSubType.getDeclaredConstructor().newInstance();
          instances.add(instance);
        } catch (InstantiationException e) {
          throw new RuntimeException("Error creating instance of type " + nextSubType.getName(), e);
